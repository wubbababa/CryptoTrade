...
- `/add_position <trade_id> <价格>`：仅在 `WAITING_ADD` 按当前已成交数量挂补仓单。
- `/close_position <trade_id>`：明确市价平仓，必须先唯一核对远程持仓数量与本交易进场数量一致。

人工指令既可直接手输，也可按交易点击按钮：一键动作（撤单、取消止损、盈利保本、保本离场、平仓）点两次确认即可；
需要价格的指令（改挂单价、改止盈价、改止损价、补仓）点击后会返回**已填好交易编号**的命令，只需补一个价格。
人工指令的编号与 Telegram 消息编号绑定，消息重投递不会造成重复下单；命令执行结果会写入
`audit_logs` 并回复到原聊天。中文快捷指令示例：`BTC 保本离场`、`BTC 保本`、`BTC 取消止损`、`BTC 补仓 59000`。


出于安全考虑，命令只对以下来源生效：来源频道，以及 `.env` 中 `TELEGRAM_COMMAND_CHAT_IDS`
显式列出的聊天（逗号分隔，通常填管理员私聊 ID）。未列入的私聊消息不会被接收，也不会被当作公告解析。
私聊中的 `/` 命令与上述中文快捷指令会进入命令层，其余文本一律忽略。
