...
- trade_id: 修改/平仓已有订单时必须填写原文中的唯一交易编号；若原文未提供，必须在 ambiguities 中说明，绝不可按币种猜测。
...
5. “挂单改价”使用 AMEND_ENTRY 并填写 entry；“止损改价/恢复止损/保本”使用 MOVE_STOP（保本未给价格时 stop_loss 为 null）；未成交撤挂单或已成交取消止损使用 CANCEL_ORDER；明确“市价平仓”才使用 CLOSE_POSITION。
"""
