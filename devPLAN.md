...
### 3.7 【已完成】人工指令的 Telegram 对话入口与弹性控制

**已完成**：`telegram_commands.TelegramCommandHandler` 从「只读命令层」升级为「对话命令层」，
新增 `/trades` 与人工指令，并由 [`main.py`](main.py) 注入同一个 `TradingService`；
`/start`、`/help`、`/status`、`/trades` 的回复均附带**内联按钮**，可点击执行。

| 命令 | 复用用例 | 前置条件 |
| :--- | :--- | :--- |
| `/amend_entry <trade_id> <价格>` | `AMEND_ENTRY` | 仅 `PENDING_ENTRY`；本地与远程同一 `client_order_id` 均存在 |
| `/cancel_order <trade_id>` | `CANCEL_ORDER` | 未成交撤进场；已开仓取消止损并转入 `WAITING_ADD` |
| `/cancel_stop <trade_id>` | `CANCEL_ORDER` | 语义化别名，明确只取消止损 |
| `/move_stop <trade_id> <价格>` | `MOVE_STOP` | `OPEN`/`WAITING_ADD`；已有止损时只允许向有利方向移动 |
| `/move_stop <trade_id> breakeven` | `MOVE_STOP` | 无价格恢复止损，按真实持仓均价浮盈 1% 重建 |
| `/amend_take_profit <trade_id> <价格>` | `AMEND_TAKE_PROFIT` | 仅 `OPEN`；先挂新止盈再撤旧止盈 |
| `/add_position <trade_id> <价格>` | `ADD_POSITION` | 仅 `WAITING_ADD`；按当前已成交数量挂限价补仓单 |
| `BTC 保本离场` | `BREAKEVEN_EXIT` | 将止盈移动到实时持仓均价浮盈 1% |
| `BTC 保本` / `BTC 恢复止损` | `MOVE_STOP` | 将止损移动到实时持仓均价浮盈 1% |
| `BTC 补仓 <价格>` | `ADD_POSITION` | 中文快捷语法，唯一匹配活动交易后挂补仓单 |
| `/close_position <trade_id>` | `CLOSE_POSITION` | 仅 `OPEN`；远程持仓数量必须与该交易进场数量完全一致 |

**可点击按钮（新增）**：

- 新增纯逻辑模块 [`telegram_menu.py`](telegram_menu.py)：以 `cb:` 前缀 + 紧凑动作编码生成
  `callback_data`，交易编号用 8 位 SHA-256 指纹（Telegram 上限 64 字节），点击后回查数据库还原真实编号，
  命中 0 条或多条一律拒绝——**绝不猜测目标交易**。
- 按钮按交易状态过滤（`STATE_ACTIONS`），未成交交易不会出现「市价平仓」这类必然被拒绝的动作。
- **写操作两步确认**：第一次点击只出确认键盘，第二次才真正调用交易所；确认按钮携带指纹而非明文编号。
- 需要价格的指令（改挂单价、改止盈价、改止损价、补仓）点击后返回**已填好交易编号**的命令模板，人工只补一个价格。
- 按钮点击与文本命令**共用同一权限边界**（来源频道或 `TELEGRAM_COMMAND_CHAT_IDS` 白名单），
  且回调命令名经过只读/写指令白名单校验，伪造 `callback_data` 无法越权。
- `telegram_client` 扩展：`reply()` 支持内联键盘，`edit_reply()` 就地刷新避免刷屏
  （失败自动退化为新消息），`answer_callback()` 应答点击，`TelegramCallback` 归一按钮更新。

安全设计：

- 命令层**只做参数解析与回复**，不直接调用交易所适配器；权限、状态与远程核对全部落在
  `TradingService._load_trade()` 和各分支，避免两套校验逻辑漂移；
- `trade_id` 是唯一目标定位手段：手输场景先按编号读取本地交易并据此填充交易所/合约/方向，
  用例层再比对一次；按钮场景由指纹回查，无法唯一命中即拒绝；
- 指令编号绑定 Telegram 消息编号（`tg-<chat>-<message>`），消息重投递因 `commands` 主键幂等而
  不会重复下单，但「失败后原样重试」不会被误判为重复指令；按钮仍复用同一人工指令路径，
  因此两种入口的执行逻辑与审计完全一致；
- 价格解析拒绝 `NaN`/`Infinity`/非正数，避免把无效输入送到交易所；
- `MOVE_STOP` 不带价格即恢复止损（`stop_loss=null`），由用例层按进场均价浮盈 1% 计算。

**验证**：`tests/test_telegram_commands.py` 覆盖命令解析、`/status` 与 `/trades` 输出、
菜单按钮 `callback_data` 的 64 字节限制与可解析性、逐笔交易按钮的状态过滤、写操作必须二次确认、
价格指令返回预填模板、指纹查不到/命中多笔的拒绝、伪造回调越权拒绝、按钮点击就地编辑消息，
以及「命令层不直接调用交易所写接口」的安全边界。

**实现说明**：修改止盈与保本离场均采用「先挂新单、后撤旧单」；补仓只允许在 `WAITING_ADD` 状态执行，数量沿用当前已成交数量，成交后按合并持仓数量扩容原止盈，止损由后续人工指令重新设定。
