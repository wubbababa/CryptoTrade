...
### 3.7 【P3】人工指令的 Telegram 对话入口（已完成对话入口与可点击按钮，仍缺改止盈/补仓）

**已完成**：`telegram_commands.TelegramCommandHandler` 从「只读命令层」升级为「对话命令层」，
新增 `/trades` 与五个人工指令，并由 [`main.py`](main.py) 注入同一个 `TradingService`；
...
- 需要价格的指令（改挂单价、改止损价）点击后返回**已填好交易编号**的命令模板，人工只补一个价格。
...
**仍待实现**：修改止盈、补仓，以及「保本离场」把止盈改为进场价浮盈 1%。
...
4. **P3**：改止盈/补仓与「保本离场」改止盈（对话入口与按钮已完成，见 §3.7）；
